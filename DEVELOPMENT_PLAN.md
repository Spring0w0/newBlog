...
- 后端数据库对受管文件引用统一保存 `/images/...` 相对公开路径，API 再根据环境 `app.upload.public-base-url` 返回绝对 URL；避免将开发机 `localhost`、反向代理内网地址或生产域名固化到业务数据。
- 已执行历史文章图片迁移：66 篇文章更新，151 个 JPEG/PNG/WebP 文件进入 `uploads/blog-images` 并建立 `file_assets` 与文章图片引用；1 个 SVG 因运行期上传安全策略被保留为历史静态资源，未放宽 SVG 限制。
...
### 迭代 C：文章与分类管理闭环（已完成，2026-07-11）

**目标**：完成 CMS 的核心写入能力——新建、编辑、删除和批量删除文章，以及分类排序。

本轮完成记录：

- 已通过 `V6__link_blog_images_to_file_assets.sql` 将文章封面和正文图片关联到 `file_assets`；文章写入时从 Markdown 图片语法提取 URL，受管 `blog-images` 会建立文件引用，外部和历史 URL 保持兼容。
- 已实现受 `ADMIN` 和 Bearer JWT 保护的文章管理分页、详情、创建、更新、单篇删除、批量删除和分类保存接口；所有更新/删除都使用数据库稳定 `id`，`slug` 只负责公开路由。
- 已补齐文章 DTO/VO、`BlogImage` Entity/Mapper、slug 与分类校验、批量删除事务、分类仍被文章引用时的 `422` 保护，以及文章/分类 Caffeine 缓存失效。
- 前端写作页、编辑加载、单篇删除、博客列表删除与分类保存已全部改走统一管理端 API；编辑页仍按 slug 打开，但会先取得文章数据库 ID。文章与分类保存成功后会重新验证公开文章与分类 SWR 数据。
- 已删除文章专用的 GitHub 索引写入适配层；其他资源的 GitHub 写入将按后续迭代分别替换，不与文章链路混用。
- 已完成真实数据库联调：临时分类创建、图片上传、文章创建、管理端分页/详情读取、更新、引用中文件删除返回 `422`、文章删除、文件删除及分类恢复均返回预期结果；临时文章、文件和上传目录均已清理。
...
进入 **迭代 D：其他内容管理闭环**。建议按“片段与关于页 → 博主、项目、友链 → 相册”的顺序推进；每类资源均先更新 [API 契约](backend/docs/API.md)，再完成 Flyway/三层后端、前端 `push-*` 替换、文件引用校验、测试与真实联调。文章与分类链路已经独立于 GitHub，可作为后续资源迁移的实现范式。

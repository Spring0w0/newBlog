...
| GitHub CMS 完全移除 | 未开始 | 首页配置与衍生产物仍在后续迭代，届时统一清理剩余 GitHub 旧链路 |
...
- 已实现 about、代码片段的管理员整体保存接口；写入后同步失效对应公开缓存。
- 已实现博主、项目、友链的管理员列表、创建、更新、删除接口；管理端使用数据库稳定 ID，公开接口不泄露管理 ID。
- 已通过 `V7__link_content_images_to_file_assets.sql` 为博主头像、项目图片、友链 Logo 增加受管文件引用，并建立相册分组与多图片的 `picture_images` 关系表；历史可匹配的受管 URL 会回填引用，外部和历史静态 URL 保持兼容。
- 已实现统一受管图片 URL 校验：提交的 `/images/**` 地址必须与资源 scope 和 `file_assets` 元数据匹配；数据库只保存规范的相对 `/images/...` 路径，接口继续返回环境对应的绝对公开 URL。
- 前端 about、片段、博主、项目、友链、相册的保存已全部替换为管理端 API；进入编辑模式先读取管理员列表取得稳定 ID，保存时分别执行创建、更新、删除，成功后重新验证公开 SWR 数据。图片上传和取消编辑时的孤儿图片清理沿用迭代 B 的统一文件接口。
- 已通过后端 29 项 Maven 测试、前端 TypeScript 类型检查与生产构建；开发库已成功应用 V7，并在独立 8082 实例验证管理员登录、六个公开读取接口、四个管理员列表接口和未登录 `401`。验证未新增或删除已有内容。

### 迭代 E：首页配置管理闭环（已完成，2026-07-12）
...
- 已通过后端 31 项 Maven 测试、前端 TypeScript 类型检查和生产构建；开发库真实联调验证“管理员登录 → 读取当前配置/卡片样式 → 原子保存 200 → 公开重新读取 200”，未登录保存正确返回 `401`。联调仅重新保存现有配置，页面展示内容保持一致；空白背景占位项被规范化为无背景项。
...
进入 **迭代 F：衍生产物、可观测性与旧链路清理**。优先清理已替换的 GitHub CMS 写入依赖与静态 JSON 主数据回退，再让 RSS、sitemap、SEO metadata 统一消费公开后端 API，并补足认证恢复、运行日志、部署和备份说明。

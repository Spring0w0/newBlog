...
| 6 | 站点配置、卡片样式后台保存 | 已实现 |
| 7 | RSS、sitemap、SEO 数据消费 | 已实现 |
| 8 | 移除 GitHub CMS 旧链路 | 已实现 |
...
请求必须携带 `Authorization: Bearer <accessToken>`；未登录、Token 无效或账号已禁用时返回 `401`。
...
| 方法 | 路径 | 状态 | `data` |
|---|---|---|---|
| GET | `/api/site/config` | 已实现 | 站点配置 JSON，包含 meta、主题、图片、社交按钮和首页开关 |
| GET | `/api/site/card-styles` | 已实现 | 以卡片名为 key 的首页布局对象 |
...
{
  "config": {
    "meta": { "title": "我的站点", "description": "...", "username": "Suni" },
    "hiCard": {
      "greeting": "",
      "introPrefix": "I'm",
...
1. 校验 `meta`、`hiCard`、主题颜色、背景颜色、帽子索引、URL、图片引用、卡片 key 和数值范围；不支持的字段不应被静默丢弃。
...
前端服务端读取统一使用 `INTERNAL_API_BASE_URL`（未设置时回退 `NEXT_PUBLIC_API_BASE_URL`），并以 300 秒为缓存窗口；公开 API 暂不可用时，RSS 会返回可用的空订阅，sitemap 至少保留首页，页面 metadata 使用内置默认站点信息。所有生产内容只从后端公开 API 读取。

## 10. 迁移、缓存与兼容性规则

### 10.1 空库初始化与首个管理员

项目不再携带静态内容导入器。新生产数据库执行 Flyway 后只会创建表结构、空内容集合、通用站点配置和默认卡片布局；文章、分类、博主、项目、友链、相册和片段均为空，必须通过 `/api/admin/**` 创建。

旧版 `V3` 已进入 Flyway 历史，不能直接改写。`V8` 会移除其固定 `admin/admin` 账号；应用在 Web 服务开始接收请求前，仅当 `users` 表为空时，使用以下生产环境变量创建首个管理员：
...
后续重启不会覆盖已有账号或已修改的密码。变量缺失时，生产配置校验失败，应用不会以固定默认密码启动。
...
| 修改资源 | 需要失效的读取数据 |
|---|---|
| 博客、分类 | 博客列表、文章详情、分类；Next.js 的 RSS、sitemap 缓存最多在 300 秒内更新 |
| 站点配置、卡片样式 | 首页配置；Next.js 的 metadata、RSS 站点信息缓存最多在 300 秒内更新 |
| 其他内容资源 | 对应公开列表 |
| 文件 | 文件元数据与引用资源缓存 |

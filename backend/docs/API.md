...
前端服务端读取统一使用 `INTERNAL_API_BASE_URL`（未设置时回退 `NEXT_PUBLIC_API_BASE_URL`），并以 300 秒为缓存窗口；公开 API 暂不可用时，RSS 会返回可用的空订阅，sitemap 至少保留首页，页面 metadata 使用内置默认站点信息。不能重新读取 `frontend/public/blogs` 或静态 JSON 作为主数据源。
